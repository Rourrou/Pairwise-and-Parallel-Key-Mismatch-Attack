         ...
         int *h, int k, int select, int block)
{
   ...
   *  Pb[0]    = rounding(q/32)  
   */
  Pb.vec[select].coeffs[0] = (int)(KYBER_Q/32.0 + 0.5);

  polyvec_reduce(&Pb);  // compress(Pb) = c1
  for(int cof = 0; cof < block && k + cof < KYBER_N; cof++){
    v.coeffs[k+cof] = h[cof];      // set c2[cof] = h[cof]
  }
         ...
         int h, int b1, int b2, int k, int select)
{
  ...
  /*  set 
   *  Pb[0]    = b1*rounding(q/32)  
   *  Pb[128]  = b2*rounding(q/32) 
   */
  Pb.vec[select].coeffs[0] = b1*(int)(KYBER_Q/32.0 + 0.5);
  Pb.vec[select].coeffs[128] = b2*(int)(KYBER_Q/32.0 + 0.5);

  polyvec_reduce(&Pb);  // compress(Pb) = c1
  v.coeffs[k] = h;      // set c2[cof] = h[cof]
